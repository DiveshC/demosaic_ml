...
import sys
from utils.support import mosaic, pad_image, get_samples, error
...
# output_name = "data/out/{}".format(output_name)
...
for x in range(int(img_data.shape[0])):
    for y in range(int(img_data.shape[1])):
        ...
        if(x%2==0 and y%2==0):
            # PATCH TYPE A - GB
            g_data[x][y] = np.sum(coeffs[0] * col[0])
            b_data[x][y] = np.sum(coeffs[1] * col[0])
        elif(x%2==1 and y%2==0):
            # PATCH TYPE B - RB
            r_data[x][y] = np.sum(coeffs[2] * col[0])
            b_data[x][y] = np.sum(coeffs[3] * col[0])
        elif(x%2==0 and y%2==1):
            # PATCH TYPE C -RB
            r_data[x][y] = np.sum(coeffs[4] * col[0])
            b_data[x][y] = np.sum(coeffs[5] * col[0])
        elif(x%2==1 and y%2==1):
            # PATCH TYPE D -RG
            r_data[x][y] = np.sum(coeffs[6] * col[0])
            g_data[x][y] = np.sum(coeffs[7] * col[0])
